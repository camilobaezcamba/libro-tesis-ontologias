...
En la ontología desarrollada se utilizó la propiedad \textit{FunctionalProperty} en aquellas que solo aceptan un objeto o valor, por ejemplo la relacion entre Planificacion(\textit{Planning}) y presupuesto(\textit{Budget}) es una relación funcional ya que existe solo un presupuesto para una planificacion específica, como muestra el Cuadro \ref{lst:planning-budget}. Asi se agregó dicha restricción a las demas relaciones que cumplian con esa condición.

\noindent\begin{minipage}[c]{\textwidth}
    \begin{lstlisting}[captionpos=b, caption=Relacion Funcional entre Planificación y presupuesto, label={lst:planning-budget},  numbers=left,  numberstyle=\tiny\color{mygray},frame=single]
