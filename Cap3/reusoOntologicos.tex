...
Para los documentos correspondientes a cada fase del proceso licitatorio se hizo una especialización de la clase Document, siendo así una subclase del mismo. Por lo tanto tenemos el enunciado de la tabla xx


    
\begin{lstlisting}[captionpos=b, caption=SPARQL query, label=lst:sparql1,  numbers=left,  numberstyle=\tiny\color{mygray},
    basicstyle=\ttfamily,frame=single]
:Document rdf:type owl:Class ;
    rdfs:subClassOf foaf:Document .
 \end{lstlisting}

 Al mismo tiempo se reutilizaron de la ontología Public Contract, los conceptos de Tender y Contract de un proceso licitatorio

 \begin{lstlisting}[captionpos=b, caption=SPARQL query, label=lst:sparql2,  numbers=left,  numberstyle=\tiny\color{mygray},
    basicstyle=\ttfamily,frame=single]
...
:Contract rdf:type owl:Class ;
 ...
 Good Relations es una ontología para modelar compras de bienes y servicios a través de internet, por lo que también se procedió a la reutilización de los conceptos gr:Offering, gr:BusinessEntity y gr:PriceSpecification.


 \begin{lstlisting}[captionpos=b, caption=SPARQL query, label=lst:sparql3,  numbers=left,  numberstyle=\tiny\color{mygray},
    basicstyle=\ttfamily,frame=single]
...
:Value rdf:type owl:Class ;
 ...
 Se reutilizó la ontología de Schema.org para conceptos básicos como los presentados a continuación.


 \begin{lstlisting}[captionpos=b, caption=SPARQL query, label=lst:sparql4,  numbers=left,  numberstyle=\tiny\color{mygray},
    basicstyle=\ttfamily,frame=single]
...
:ContactPoint rdf:type owl:Class ;
 ...
 Además se utilizó la ontología Sequence para modelar la relación de secuencia que existe dentro del proceso licitatorio. En el cuadro muestra como ejemplo la propiedad que indica que la Planificación precede directamente al Llamado.



 \begin{lstlisting}[captionpos=b, caption=SPARQL query, label=lst:sparql5,  numbers=left,  numberstyle=\tiny\color{mygray},
    basicstyle=\ttfamily,frame=single]
...
Con esto se concluyó con la reutilización de ontologías, paso necesario para poder enriquecer la semántica de la ontología desarrollada. La figura xx es una representación visual en forma de grafo de la ontología desarrolla. A continuación, se procederá al uso de la ontología desarrollada y posteriores pruebas con la misma.
